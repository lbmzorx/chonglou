...
class LoginForm extends Model
{
    ...
    {
        $parent= parent::beforeValidate(); // TODO: Change the autogenerated stub
        $this->rsa->decryptModelAttr($this,'password');
        return $parent;
    }
            ...
            if( $this->checkPasswordCache($cache,$key) ){
                if ($user){
                    if( $user->status==User::STATUS_LOGINERROR && (time()-$user->edit_time)>72*3600){
                        $this->addError($attribute, Yii::t('app','还有').(($user->edit_time)*72*3600-time()). Yii::t('app','秒才可重新登录'));
